...
function AllocationSettingsAllocationPreference() {
  const { userPreferences } = useSelector(
    (state: RootState) => state.userPreferences
  );
  const { user } = useSelector((state: RootState) => state.user);

  const [currentAllocationPreference, setCurrentAllocationPreference] =
    useState<string>(DEFAULT_ALLOCATION_PREFERENCE);
  ...
  useEffect(() => {
    setCurrentAllocationPreference(
      (userPreferences?.Allocation_Preference ||
        DEFAULT_ALLOCATION_PREFERENCE) as string
    );
  }, [userPreferences]);

  const handleUpdateAllocationPreference = () => {
    if (!user) return;
    if (currentAllocationPreference) {
        ...
        dispatch({
          ...
          payload: {
            postData: {
              ...
              Value: currentAllocationPreference,
            },
            resolve,
            reject,
          },
  ...
  return (
    ...
    >
      ...
      >
        ...
        <FormControl variant="outlined" sx={{ minWidth: 200 }}>
          <Select
            value={currentAllocationPreference}
            onChange={e =>
              setCurrentAllocationPreference(e.target.value as string)
            }
            displayEmpty
      ...
      >
        ...
        <Button
          disabled={
            currentAllocationPreference ===
            userPreferences?.Allocation_Preference
          }
          variant="contained"
          onClick={handleUpdateAllocationPreference}
          sx={{
            height: 40,
            borderRadius: 2,
            background: '#152E75',
...
export default AllocationSettingsAllocationPreference;
