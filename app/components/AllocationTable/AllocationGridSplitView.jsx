...
export default function AllocationGridSplitView({
  groupBy,
  columns,
  data,
  loading,
  ...
  useEffect(() => {
    if (currentView?.ColumnsVisible && groupBy) {
      ...
      console.log('useEffect, updatedModel : ', updatedModel);
  ...
  const handleColumnVisibilityModelChange = newModel => {
    ...
    console.log('handleColumnVisibilityModelChange : ', updatedModel);
    setColumnVisibilityModel(updatedModel);
    dispatch(
      updateCurrentView({
  ...
  return (
    <Box sx={{ height: 'calc(100vh - 54px)', width: '100%' }}>
      <StyledDataGrid
        ...
        // slots={{
        //   // toolbar: ToolbarMod,
        //   ...(toolbarComponent ?? { toolbar: CustomToolbar }),
